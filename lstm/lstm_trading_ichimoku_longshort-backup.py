...
Adds short-selling to the original long-only backtester with strict
single-position enforcement (no overlaps). Decisions at bar t execute at
the next bar open (t+1). Intrabar SL/TP uses conservative stop-first priority.

Artifacts (saved to --out-dir):
- model:   lstm_ichimoku_{TICKER}.pt
- preds:   lstm_ichimoku_preds_train_{TICKER}.csv
- equity:  lstm_ichimoku_equity_test_{TICKER}.csv
- trades:  lstm_ichimoku_trades_{TICKER}.json
- summary: lstm_ichimoku_summary_{TICKER}.json
...
def backtest_threshold_ls(df_raw: pd.DataFrame, df_feat: pd.DataFrame, feature_cols: List[str],
    ...
    for j in idxs:
        ...
        seq = (seq - mean) / std
        prob = float(predict_proba(model, seq, device=device)[0])  # P(up)
        ...
        # ------ Flat: consider entries (no overlap) ------
        if pos == 0:
            take_long = prob >= pos_thr
            take_short = prob <= neg_thr

            # If both thresholds are crossed, choose the stronger distance from threshold
            if take_long and take_short:
                dist_long = abs(prob - pos_thr)
                dist_short = abs(neg_thr - prob)
                take_long = dist_long >= dist_short
                take_short = not take_long

            if take_long:
                pos = +1
                entry_px = o1
                equity *= fee_mult
                trades.append({"side": "LONG", "entry_ts": str(ts1), "entry_px": entry_px, "prob": prob})
            ...
            if take_short:
                ...
                entry_px = o1
                equity *= fee_mult
                trades.append({"side": "SHORT", "entry_ts": str(ts1), "entry_px": entry_px, "prob": prob})
                eq_curve.append((ts1, equity))
                continue
        ...
        if pos == +1:
            ...
            # signal exit if probability falls below pos_thr
            if not exit_now and prob < pos_thr:
        ...
        if pos == -1:
            ...
            # signal exit if probability rises above neg_thr (i.e., not bearish)
            if not exit_now and prob > neg_thr:
                exit_now = True; exit_px = o1; reason = "SIG"

            if exit_now:
                qty = equity / entry_px
                pnl = (entry_px - exit_px) * qty  # reverse PnL for short
                equity += pnl
                equity *= fee_mult
                pos = 0
                trades[-1].update({"exit_ts": str(ts1), "exit_px": exit_px, "reason": reason, "pnl": pnl})
...
def main():
    ...
    preds_train = pd.DataFrame({"timestamp": t_end_train, "prob_up": p_tr, "label": y_train_all})
    ...
    if args.test_end:
        ...
        eq, trades = backtest_threshold_ls(
            df_all, feat_all, feature_cols, model, mean, std,
            lookback=args.lookback, pos_thr=args.pos_thr, neg_thr=neg_thr,
    ...
    if args.bundle_dir:
        bdir = os.path.abspath(os.path.expanduser(args.bundle_dir))
        os.makedirs(bdir, exist_ok=True)

        # 1) model.pt (rename/copy)
        shutil.copy2(mdl_path, os.path.join(bdir, "model.pt"))

        # 2) preprocess.npz (mean/std from the checkpoint you just saved)
        np.savez(os.path.join(bdir, "preprocess.npz"), mean=mean, std=std)

        # 3) meta.json (SAT-friendly metadata)
        meta = {
            "type": "lstm_ichimoku",
            "version": "1.0",
            ...
            "thresholds": {"pos_thr": args.pos_thr, "neg_thr": float(neg_thr)},
