...
# ---------- Features ----------
def rolling_mid(h: pd.Series, l: pd.Series, n: int) -> pd.Series:
    n=int(n); return (h.rolling(n,min_periods=n).max()+l.rolling(n,min_periods=n).min())/2.0
def ichimoku(df, tenkan,kijun,senkou):
    d=df.copy()
    d["tenkan"]=rolling_mid(d.high,d.low,int(tenkan))
    d["kijun"]=rolling_mid(d.high,d.low,int(kijun))
    d["span_a"]=(d["tenkan"]+d["kijun"])/2.0
    d["span_b"]=rolling_mid(d.high,d.low,int(senkou))
    d["chikou"]=d["close"].shift(-int(kijun))
    return d
...
def build_features(df, tenkan,kijun,senkou,displacement, slope_window=8):
    ...
    d["ts"]=df["ts"].values; d["timestamp"]=df["timestamp"].values
    return d

# ---------- Bundle ----------
def load_bundle(model_dir: str):
    model_dir=os.path.abspath(os.path.expanduser(model_dir))
    meta_path=os.path.join(model_dir,"meta.json"); pre_path=os.path.join(model_dir,"preprocess.json"); mdl_path=os.path.join(model_dir,"model.pt")
    for pth in (meta_path,pre_path,mdl_path):
        if not os.path.exists(pth): raise SystemExit(f"Missing bundle file: {pth}")
    meta=json.load(open(meta_path)); pre=json.load(open(pre_path))
    ...
    model=torch.jit.load(mdl_path,map_location="cpu"); model.eval()
...
def run_model(model, X, mean, std):
    Xn=(X-mean)/(std+1e-12)
    with torch.no_grad():
        t=torch.from_numpy(Xn).float(); out=model(t)
